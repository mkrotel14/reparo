...
jest.mock('expo-router/stack', () => {
  ...
  const Stack = ({ children }: { children?: ReactNode }) => <MockView>{children}</MockView>;
  Stack.Screen = ({ name }: { name: string }) => <MockText>Stack screen: {name}</MockText>;
  return { Stack };
});
...
const noSession = { session: null, selectRole: jest.fn(), signOut: jest.fn(), status: 'unauthenticated' as const };
const clientSession = { session: { createdAt: '2026-08-01T00:00:00.000Z', identityId: 'client-id', role: 'client' as const }, selectRole: jest.fn(), signOut: jest.fn(), status: 'authenticated' as const };
