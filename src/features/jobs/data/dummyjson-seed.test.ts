...
describe('DummyJSON jobs seed', () => {
  ...
  it('imports only the demo Client todos and records the completed seed', async () => {
    ...
    await expect(seedJobsFromDummyJson({ clientId: 'client-id', fetchImpl })).resolves.toEqual({ didSeed: true, importedCount: 1 });
    expect(transaction.runAsync).toHaveBeenNthCalledWith(1, expect.stringContaining('INSERT OR IGNORE INTO jobs'), 'dummyjson-todo-1', 1, 'client-id', 'Fix sink', 'open', expect.any(String), expect.any(String));
    expect(transaction.runAsync).toHaveBeenNthCalledWith(2, expect.stringContaining('INSERT INTO app_metadata'), 'jobs.dummyjson.seed.v1', expect.any(String));
  });

  it('does not call the network again after a successful seed', async () => {
    database.getFirstAsync.mockResolvedValue({ value: '2026-08-01T00:00:00.000Z' });
    const fetchImpl = jest.fn();

    await expect(seedJobsFromDummyJson({ clientId: 'client-id', fetchImpl })).resolves.toEqual({ didSeed: false, importedCount: 0 });
