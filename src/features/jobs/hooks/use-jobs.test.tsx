...
jest.mock("@/features/jobs/data/dummyjson-seed", () => ({
  seedJobsFromDummyJson: jest
    .fn()
    .mockResolvedValue({ didSeed: false, importedCount: 0 }),
}));
...
jest.mock("@/features/session/data/session-repository", () => ({
  sessionRepository: {
    getIdentity: jest
      .fn()
      .mockResolvedValue({ identityId: "client-1", role: "client" }),
  },
}));
...
describe("jobs hooks", () => {
  ...
  it("loads jobs through the repository", async () => {
    mockedRepository.list.mockResolvedValueOnce([
      {
        id: "1",
        title: "Fix door",
        location: "Centro",
    ...
    const { result } = await renderHook(() => useJobs(), {
  ...
  it("claims a job for the active pro", async () => {
    const { result } = await renderHook(() => useClaimJob(), {
      wrapper: createWrapper(),
    });
