...
export function JobDetailScreen({ jobId }: { jobId: string }) {
  ...
  const assignment = session.role === 'pro' ? 'You claimed this repair job.' : job.proId ? 'A Pro has claimed this repair job.' : 'Waiting for a Pro to claim this repair job.';

  return <AppScreen><View style={styles.content}><Text style={styles.title}>{job.title}</Text><AppTag tone={job.status === 'completed' ? 'success' : job.status === 'claimed' ? 'warning' : 'neutral'}>{job.status}</AppTag>{job.description ? <Text style={styles.description}>{job.description}</Text> : null}<Text style={styles.meta}>{job.location} · R${job.budget}</Text><Text style={styles.meta}>{assignment}</Text></View></AppScreen>;
}
