...
describe('<ProfileScreen />', () => {
  ...
  it('shows the account home and exposes logout without a role switch', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Language' })).not.toBeOnTheScreen();
    expect(screen.queryByRole('button', { name: 'App version' })).not.toBeOnTheScreen();
  ...
  it('opens device settings when the platform supports the handoff', async () => {
    ...
    mockedUseSession.mockReturnValue({ session: { createdAt: '2026-08-01T00:00:00.000Z', identityId: 'client-uuid', role: 'client' }, selectRole: jest.fn(), signOut: jest.fn(), status: 'authenticated' });
    mockedUseProfileSummary.mockReturnValue({
      identity: { displayName: 'Reparo Client', email: 'client@reparo.local', localId: 'client-uuid', roleLabel: 'Client' },
      isLoading: false,
      summary: { primaryMetric: { label: 'Requests completed', value: 1 }, supportingMetrics: [{ label: 'Open requests', value: 0 }, { label: 'In progress', value: 0 }] },
    });
