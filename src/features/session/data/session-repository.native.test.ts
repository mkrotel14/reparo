...
describe('native session repository', () => {
  ...
  it('creates an identity and upserts it as the active role session', async () => {
    ...
    await expect(sessionRepository.selectRole('pro')).resolves.toMatchObject({ identityId: 'pro-uuid', role: 'pro' });

    expect(database.runAsync).toHaveBeenNthCalledWith(1, expect.stringContaining('INSERT OR IGNORE INTO local_identities'), 'pro', 'pro-uuid', null, expect.any(String));
    expect(database.runAsync).toHaveBeenNthCalledWith(2, expect.stringContaining('ON CONFLICT(singleton) DO UPDATE'), 'pro', 'pro-uuid', null, expect.any(String));
  });

  it('clears only the active session row', async () => {
    await sessionRepository.clear();

    expect(database.runAsync).toHaveBeenCalledWith('DELETE FROM active_session WHERE singleton = 1');
    expect(database.runAsync).not.toHaveBeenCalledWith(expect.stringContaining('local_identities'));
  });
});
